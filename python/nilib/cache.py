...
Designed for use with Twisted framework using asynchronous programming
techniques.  Thus there is minimal locking but operations are designed to run
very rapidly - no blocking and minimal delay.  
...
1.0       30/11/2012 Elwyn Davies   Provide separate dictionary and shared
...
__all__ = ['InconsistentParams', 'InvalidMetaData', 'CacheEntryExists',
           'NoCacheEntry', 'NetInfCache']
...
class NetInfCache:
    ...
    def cache_get(self, ni_name):
        ...
        - if so, read metatdata and put into NetInfMetaData object instance
...
if __name__ == "__main__":
    ...
    cache_inst = NetInfCache(storage_root, logger)
    f = open(cache_inst.temp_path+"temp", "w")
    f.write("some_text")
    f.close()
    cache_inst2 = NetInfCache(storage_root, logger)
